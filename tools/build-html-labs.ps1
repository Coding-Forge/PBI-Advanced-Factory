...
.column-table code { font-family: Consolas, "Courier New", Courier, monospace; }
...
$modules = @(
  ...
  @{
    ...
    Tasks = @(
      ...
      @{ Title = "Add parameters and functions"; Body = @("Create RawDataBaseUrl.", "Create fn_CleanText for reusable null-safe text cleanup.") },
    ...
    Checklist = @("Web URLs loaded", "Monthly queries appended", "Explicit types applied", "Data quality issues found", "Custom function created", "Incremental refresh notes captured")
  },
  @{
...
<section>
  <h2>Novice-friendly how-to guide</h2>
  <div class="task-list">
    <div class="task-card"><h3>Create a Power Query parameter</h3><ol><li>Open <strong>Transform data</strong> to launch Power Query.</li><li>Select <strong>Manage Parameters &gt; New Parameter</strong>.</li><li>Enter the exact parameter name, data type, and current value from the lab.</li><li>Select <strong>OK</strong> and use the parameter in source steps instead of hard-coded paths.</li></ol></div>
...
<tr><td><code>fact_Orders</code></td><td>Reference staging query</td><td>Yes</td><td>Typed columns: <code>OrderId</code>, <code>OrderDate</code>, <code>CustomerName</code>, <code>ProductCode</code>, <code>Quantity</code>, <code>UnitPrice</code>, <code>SalesChannel</code>, <code>SourceFile</code>.</td></tr>
...
<section>
  ...
  <ol class="steps">
    ...
    <li><strong>Create a raw data base parameter.</strong><p>Create a text parameter named <code>RawDataBaseUrl</code> with value <code>https://raw.githubusercontent.com/Coding-Forge/PBI-Advanced-Factory/main/data/</code>. This makes the Web source pattern easier to maintain.</p></li>
    ...
    <li><strong>Create the final fact query.</strong><p>Reference <code>stg_OrdersCombined</code> and name the new query <code>fact_Orders</code>. Apply explicit data types for OrderId, OrderDate, ProductCode, Quantity, UnitPrice, and SalesChannel.</p></li>
    <li><strong>Create a cleanup function.</strong><p>Create <code>fn_CleanText</code> to trim and clean nullable text values. Invoke it on CustomerName and other text columns where whitespace or inconsistent capitalization exists.</p></li>
    <li><strong>Identify bad data.</strong><p>Use column quality/profile and type-conversion errors to find the intentional bad date and bad quantity in the March file. Create an error review query before removing or replacing rows.</p></li>
    <li><strong>Load reference data.</strong><p>Use the Web connector for <code>product-category-map.csv</code>. Merge it into <code>fact_Orders</code> on ProductCode or keep it as a dimension/reference table, depending on the lab design.</p></li>
    <li><strong>Prepare for incremental refresh.</strong><p>Create DateTime parameters named <code>RangeStart</code> and <code>RangeEnd</code>. Filter the final fact query on OrderDate using those parameters. Document Service-side incremental refresh as Verify for Gov.</p></li>
  </ol>
</section>
'@ }
    "04" { return @'
...
<section>
  <h2>Detailed step-by-step procedure</h2>
  <ol class="steps">
    <li><strong>Start with the Gov-ready core.</strong><p>Open the report PBIP and choose a page where margin or sales scenario analysis makes sense.</p></li>
    <li><strong>Create a what-if parameter.</strong><p>In Power BI Desktop, create a parameter named Margin Adjustment %. Use a reasonable range such as -10 to 20 with an increment such as 1.</p></li>
    <li><strong>Create adjusted measures.</strong><p>Create a measure that applies the selected parameter to Gross Margin or Sales Amount. Use a clear name such as Adjusted Gross Margin.</p></li>
    <li><strong>Add scenario visuals.</strong><p>Add the parameter slicer, a base metric card, an adjusted metric card, and a comparison visual. Confirm visuals update when the slicer changes.</p></li>
    <li><strong>Evaluate decomposition tree.</strong><p>If available, add the visual and use Sales Amount or Gross Margin as the analyzed metric. Add dimensions such as Territory, Product Category, Customer Type, and Segment. If unavailable, use a matrix and drillthrough instead.</p></li>
    <li><strong>Evaluate forecasting/anomaly options.</strong><p>If available, add forecasting or anomaly detection to a supported line chart. If unavailable, create rolling averages, prior-period comparison, or DAX threshold flags.</p></li>
    <li><strong>Evaluate key influencers.</strong><p>If available, test the visual with a clear outcome and explanatory fields. Discuss interpretation limits. If unavailable, use ranking and Top N visuals.</p></li>
    <li><strong>Review Python/R and Azure ML.</strong><p>Do not add these unless runtimes, packages, Service support, network, identity, and data residency are validated. Use native visuals or static scored sample outputs as the fallback.</p></li>
    <li><strong>Review Copilot conceptually.</strong><p>If Copilot is not validated, use the conceptual section only. Explain that AI output requires human review and cannot be treated as authoritative.</p></li>
    <li><strong>Document feature status.</strong><p>For every advanced or AI-assisted feature, record Gov-ready, Verify for Gov, or Commercial-focused and list the fallback used.</p></li>
  </ol>
</section>
'@ }
    "07" { return @'
...
<section>
  ...
  <ol class="steps">
    ...
    <li><strong>Review deployment pipelines.</strong><p>If available, map dev/test/prod workspaces and review deployment rules. If unavailable, document the manual promotion path.</p></li>
    <li><strong>Complete endorsement checklist.</strong><p>Review ownership, refresh, security, quality, and support evidence before marking content Promoted or Certified.</p></li>
  </ol>
</section>
'@ }
    "09" { return @'
...
<body>
  <div class="shell">
    ...
    <nav class="bottomnav">$prevLink$nextLink</nav>
